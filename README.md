...
### 7. Автозапуск на уровне машины
Чтобы бот запускался после перезагрузки без входа пользователя, установите задачу планировщика от имени администратора:
```powershell
.\install_boot_service.ps1
```
Это создаст задачу `VacancyBot24x7`, которая стартует при загрузке системы от имени `SYSTEM` и поднимает Docker-стек.
Для удаления:
```powershell
.\uninstall_boot_service.ps1
```
